...
class CommitCountData{
	public String [] names = {"Edward Thomson", "Carlos Mart\u00edn Nieto" , "Patrick Steinhardt" , "Sven Strickroth"
	,"Vicent Marti" ,"nulltoken" ,"Vicent Mart\u00ed" ,  "Philip Kelley" ,  "Russell Belfer", "Michael Schubert"
	,"Ramsay Jones","Ben Straub","Etienne Samson","Jacques Germishuys","lhchavez"};
	public Number[] counts = { 3665 ,2026,  1421,  103,  273,  589,  1758, 109, 1127,  157,  107,  541,  252,  110, 106};

	public String[] getNamesArray(){
		return names;
	}
	public Number[] getCountsArray(){
		return counts;
	}
...
class DataController {
	...
	public String barGet() {
		return handlebarRetrieve("bars","ECharts Java");
	}
	...
	public static String readJSONCommitData(ClassPathResource resource){
		//CommitCountData data = new CommitCountData();
		String data ="";
			try {
			File resource1 = new ClassPathResource("CommitCount.json").getFile();
			String text = new String(Files.readAllBytes(resource1.toPath()));
			String rows[] = text.split(",",0);
			for(int i =0;i<rows.length;i++){
				String row[] = rows[i].split(":",0);
				data = data+ " " + row[0];
				// data.names.add(row[0]);
				// data.counts.add(Integer.parseInt(row[1]));
			}
	...
	@GetMapping("/barchart")
	public ResponseEntity<String> getbarChart() {

		ClassPathResource staticDataResource = new ClassPathResource("CommitCount.json");
		String data = readJSONCommitData(staticDataResource);
		CommitCountData dat = new CommitCountData();
		String[] namesArray = dat.getNamesArray();
		Number [] countArray = dat.getCountsArray();
		...
		.addXAxis(namesArray)
		.addYAxis()
		.addSeries("Number of Commits", countArray);
    Engine engine = new Engine();
